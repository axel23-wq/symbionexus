...
export default function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!listing) return <div style={{ textAlign: 'center', padding: '80px' }}>{t('mdet.notFound')}</div>;
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ marginBottom: '24px' }}>
        <button onClick={() => router.back()} className="btn-ghost">← {t('mdet.back')}</button>
      </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
            <div>
              <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                <span className="badge badge-info">{t('cat.' + listing.materialCategory).startsWith('cat.') ? listing.materialCategory : t('cat.' + listing.materialCategory)}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '40px', marginTop: '40px' }}>
            ...
            <div>
              ...
              {listing.chemicalProfile ? (
                ...
                <p style={{ color: 'var(--color-text-muted)', fontStyle: 'italic' }}>{t('mdet.noChem')}</p>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div className="glass-card" style={{ padding: '24px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div>
                    <div style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('mkt.volume')}</div>
                  ...
                  <div>
                    <div style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('mdet.availability')}</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 600 }}>{listing.frequency ? t('freq.' + listing.frequency) : ''}</div>
                  </div>
                  <div>
                    <div style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('mdet.priceAsked')}</div>
                    <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#f59e0b' }}>
                      {listing.pricePerKg ? `${listing.pricePerKg} FCFA / kg` : t('mkt.negotiable')}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card" style={{ padding: '24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span className="badge badge-success">{t('mdet.verified')}</span>
                  <span style={{ fontSize: '0.85rem', color: 'var(--color-primary-400)', fontWeight: 600 }}>
                    ⭐ {t('dash.trustScore')} : {listing.company?.trustScore ? (listing.company.trustScore * 100).toFixed(0) : 50}%
                  </span>
                </div>
              </div>
            </div>
